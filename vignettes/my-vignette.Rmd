...
```{r QDA Error, echo = FALSE, warning=FALSE, results='hide', fig.keep='none'}
liste2 <- plot_error(set, func_name2) ##Saving the QDA Errorplot into an object
```

```{r QDA Errorplot, echo = FALSE, warning=FALSE, results='hide', fig.cap="\\label{fig:figs} Error in the QDA function. For further explanations of the barplots see LDA Error"}
...
```{r PDA, warning = FALSE, results='hide', fig.cap="\\label{fig:figs}Classification of randomly generated data with QDA. Note that the data is the same as in LDA above. The syntax of the call remains almost identical."}
...
```{r PDA Error, echo = FALSE, warning=FALSE, results='hide', fig.keep='none'}
liste3 <- plot_error(set, func_name3) ##Saving the PDA Errorplot into an object
```

```{r PDA Errorplot, echo = FALSE, warning=FALSE, results='hide', fig.cap="\\label{fig:figs} Error in the QDA function. For further explanations of the barplots see LDA Error"}
...
What happens when neither LDA or QDA sufficently classify but PDA is too complex to apply. Regularized Discriminant Analysis (RDA) is an approach to combine the best of both. Instead of using a completely pooled covariance matrix or different covariance matrices for every class RDA takes a convex conjugation of both fine-tunes the paramter of the conjugation to get the best result. If the parameter is 1 RDA is the same as LDA while if the parameter is 0 RDA is the same as QDA. Pro:

## Support Vector Machines (SVM)

A different approach to the whole problem is the Support Vector Machines