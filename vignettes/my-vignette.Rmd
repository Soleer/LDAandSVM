...
Suppose we have a data-frame of 289 observations of skyrockets for sylvester where we have measured both the heigth and the volume (as in loudness) of our rckets. There are 115 rockets of the type "Barock", 78 of the type "Renaissance" and 96 of the type "Classic". The data-frame is structured like this:
...
If we want to see how good the function has classified the **training** data (There is no test data) we create another plot object and call it:
...
We are now very close to calling the police for such a violation of the law but to make sure we want QDA to give us the same result as a confirmation because the point is very close at the edge. So we redo the proceedure with QDA
...
This package comes with a shiny app to make it easier for the user to navigate the package. We can open it the following way. See **?classify_app** for more information.
...
If we want to simply look at beautiful colored plots without any meaning behind them it is possible to create random test with this package (see **make_test** or **?make_testset** for some more information). These functions create uncorrelated normal distributed data that we can test our classifiers with. We can adjust the number of observations per class, the number of dimensions for the observations, the number of classes and the standard deviations (these get used in all directions):
...
As we can see the classes are seperated by quadratic lines which are in some cases combined to be almost elliptical (red and purple). In the example below the total error is 1% less as for LDA but empirical data shows that is far from beeing a typical result. QDA has performed among the top three in 4 of 22 datasets in the STATLOG project (see Hastie et al. "The Elements of Statistical Learning" (2009) p. 111)
...
To fight overfitting Hastie et al. have proposed a penalized metric where they add a penalty matrix Omega to the covariance matrix. The exact value of Omega has to be found by hand, cross validation, or superb intuition. As a second extra PDA does if needed is a basis expansion: "quad" for quadratic polynomial, "cube" for cubic polynomial, "sqrt" for squareroot, "log" for logarithm, "abs" for the absolute value or "id" for no basis expansion at all are implemented in this package.
